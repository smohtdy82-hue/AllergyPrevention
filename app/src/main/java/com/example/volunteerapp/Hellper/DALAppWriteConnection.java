...
public class DALAppWriteConnection {
    ...
    private static final String PROJECT_ID = "6907346e0016a09aca5d";
    private static final String API_KEY = "standard_";
    private static final String MAIN_DATABASE_ID = "690c63da00177332f216"; // AppDb
    /** بدون مسافات زائدة — مسافة في المعرف تكسر مسار رفع/تحميل الملفات */
    private static final String MAIN_STORAGE_BUCKET_ID = "690c63fe0039ec323fb6"; // AppWriteStorage
        ...
        try {
            ...
            for (T item : dataList) {
                try {
                    ...
                    normalizeDocumentDataForAppwrite(item, documentData);
    ...
    public <T> OperationResult<T> updateData(T data, String tableName, String documentId, String collectionId) {
        try {
            ...
            normalizeDocumentDataForAppwrite(data, documentData);
            // لا نرسل updatedAt / updatedBy — مخطط Appwrite للمجموعات الحالية لا يعرّف هذه السمات
            ...
            if (updated) {
                ...
                String detail = (lastDocumentSaveError != null && !lastDocumentSaveError.isEmpty())
                        ? lastDocumentSaveError : "فشل في تحديث المستند";
    ...
    /** حقول لا تُرسل في حقل data لـ Appwrite (تمت معالجة id مسبقاً إلى organizationId وغيره) */
    private Map<String, Object> filterDocumentDataForAppwrite(Map<String, Object> documentData) {
    ...
    /**
     * تحديث مستند موجود (PATCH) — يجب استخدامه مع updateData؛ POST يسبب 409 إن وُجد المستند.
     */
    private boolean patchDocument(String tableName, String collectionId, String documentId, Map<String, Object> documentData) {
        try {
            String coll = collectionId != null ? collectionId : tableName;
            URL url = new URL(BASE_URL + "/databases/" + MAIN_DATABASE_ID + "/collections/"
                    + coll + "/documents/" + documentId);
            HttpURLConnection connection = (HttpURLConnection) url.openConnection();
            ...
            Map<String, Object> cleanData = filterDocumentDataForAppwrite(documentData);
            ...
            Log.e(TAG, "patchDocument error: " + errorResponse);
            if (errorResponse.contains("Unknown attribute") || errorResponse.contains("document_invalid_structure")) {
                stripUnknownAttributes(errorResponse, cleanData);
                return patchDocumentMinimal(tableName, collectionId, documentId, cleanData);
            }
            return false;
        } catch (Exception e) {
            lastDocumentSaveError = e.getMessage() != null ? e.getMessage() : e.getClass().getSimpleName();
    ...
    private void stripUnknownAttributes(String errorResponse, Map<String, Object> data) {
        try {
            java.util.regex.Matcher m = java.util.regex.Pattern
                    .compile("Unknown attribute[:\\s\\\\\"]*([a-zA-Z_]\\w*)")
                    .matcher(errorResponse);
            while (m.find()) {
                String attr = m.group(1);
                data.remove(attr);
                Log.d(TAG, "stripUnknownAttributes: removed '" + attr + "'");
            }
        } catch (Exception ignored) {}
    }

    private boolean patchDocumentMinimal(String tableName, String collectionId, String documentId, Map<String, Object> cleanData) {
        try {
            String coll = collectionId != null ? collectionId : tableName;
            URL url = new URL(BASE_URL + "/databases/" + MAIN_DATABASE_ID + "/collections/"
                    + coll + "/documents/" + documentId);
            ...
            Map<String, Object> data = new HashMap<>(cleanData);
            ...
            lastDocumentSaveError = readErrorResponse(connection);
            Log.e(TAG, "patchDocumentMinimal error: " + lastDocumentSaveError);
            return false;
        } catch (Exception e) {
            lastDocumentSaveError = e.getMessage() != null ? e.getMessage() : e.getClass().getSimpleName();
            return false;
    ...
    private boolean saveDocument(String tableName, String collectionId, String documentId, Map<String, Object> documentData) {
        try {
            ...
            String jsonBody = gson.toJson(requestBody);
            Log.d(TAG, "saveDocument BODY >> " + jsonBody);
    ...
    private boolean saveWithoutSchemaValidation(String tableName, String collectionId, String documentId, Map<String, Object> documentData) {
        try {
            ...
            Log.d(TAG, "saveWithoutSchemaValidation BODY >> " + jsonBody);
    ...
    /**
     * تنظيف الحقول وتسمية id حسب نوع الكائن (مثل organizationId) كما يتوقع مخطط Appwrite.
     * يجب استدعاؤها قبل saveDocument في المسارات التي تستخدم convertObjectToMap مباشرة (مثل updateData).
     */
    private void normalizeDocumentDataForAppwrite(Object item, Map<String, Object> documentData) {
        if (item == null || documentData == null) return;
        Log.d(TAG, "normalizeDoc BEFORE keys: " + documentData.keySet());
        ...
        Log.d(TAG, "normalizeDoc AFTER keys: " + documentData.keySet());
    }
    ...
    public OperationResult<FileInfo> uploadFile(byte[] fileData, String fileName, String mimeType, String bucketId) {
        try {
            ...
            String actualBucketId = trimBucketId(bucketId != null ? bucketId : MAIN_STORAGE_BUCKET_ID);
    ...
    private static String trimBucketId(String bucketId) {
        if (bucketId == null) {
            return "";
        }
    ...
    /**
     * رابط تحميل ملف من التخزين (نفس الصيغة المستخدمة بعد الرفع).
     * يُستدعى من النموذج عند تخزين معرف الملف فقط في الحقل المحدود الطول (مثل imageUrl 255).
     */
    public static String publicStorageDownloadUrl(String fileId) {
        if (fileId == null || fileId.isEmpty()) {
            return null;
        }
        String bucket = trimBucketId(MAIN_STORAGE_BUCKET_ID);
        return BASE_URL + "/storage/buckets/" + bucket + "/files/" + fileId + "/download?project=" + PROJECT_ID;
    }
